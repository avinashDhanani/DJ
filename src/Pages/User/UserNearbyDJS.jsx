...
const UserNearbyDJS = () => {
  ...
  const onLikeBtnClick = (djId, islike = false, index) => {
    const payload = {
      djId,
    };
    if (islike) {
      axios
        ...
        .then((res) => {
          let tempLike = arrIsLike;
          tempLike[index] = false;
          setArrIsLike(tempLike);
        })
        .catch((err) => {});
    } else {
      axios
...
const DJDataShow = ({ el, i, token, genre, handleSingleDJ }) => {
